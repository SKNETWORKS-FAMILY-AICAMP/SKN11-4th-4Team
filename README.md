...
| 유경  | 이번 프로젝트는 프론트엔드, 백엔드, 배포에 초점을 맞추다보니 예상치 못한 네트워크 문제들을 많이 마주했었는데, 역시 네트워크는 설정은 어렵다는걸 다시금 깨달았습니다. 그래도 네트워크 문제들을 잘 해결해서 서버 간 통신을 해결해내 뜻깊었던 프로젝트였습니다! 다들 너무 수고 많으셨어요🤩 |
| 성지  | 이번 4차 프로젝트를 통해 프론트엔드와 Django를 함께 사용하면서, 양쪽 영역에 대한 이해도가 크게 향상되었습니다 특히 백엔드에 깊은 관심이 있었기 때문에, Django의 요청·응답 흐름, ORM을 활용한 데이터베이스 연동에 대해 많이 배울 수 있었습니다AWS에 Django 애플리케이션을 배포하고 RDS(MySQL)와 연결하는 과정에서는 여러 차례 실패를 겪었지만, 하나씩 원인을 분석하며 설정을 수정해 나간 덕분에 최종적으로 안정적인 통신 환경을 구축하게 되었습니다. 앞으로도 이 경험을 바탕으로, 더욱 견고하고 확장성 있는 백엔드 시스템을 설계하는 데 자신감을 가질 수 있을 것 같습니다. |
